...
// Ask the kernel to back large host allocations with transparent huge pages.
static void advise_huge_pages(uintptr_t addr, size_t size) {
	constexpr size_t HUGEPAGE_MIN_SIZE = 2u * 1024u * 1024u;
	if (size >= HUGEPAGE_MIN_SIZE) {
		::madvise(reinterpret_cast<void*>(addr), size, MADV_HUGEPAGE);
	}
}
...
uint64_t Alloc(uint64_t address, uint64_t size, Mode mode) {
	...
	if (ptr != MAP_FAILED) {
		advise_huge_pages(ret_addr, size);
...
// Query /proc/self/maps to decide whether a range is already occupied.
